...
async def read_fortune_track(db:Session=Depends(get_db)):
	data = await get_track_from_spotify()
	create_history(db=db, spotify_song_id=data['id'])
	return data


@router.get("/tracks/recommend/authed")
async def read_fortune_track(db:Session=Depends(get_db), jwt_payload=Depends(JWTBearer())):
	data = await get_track_from_spotify()
